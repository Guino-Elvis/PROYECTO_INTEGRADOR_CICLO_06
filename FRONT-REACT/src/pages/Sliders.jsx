...
import { saveFile ,  deleteFile} from "../firebase/firebaseSlider";// Importa los métodos de Firebase
...
// end paginacion
 ...
 const hasNextPage = () => {
  return currentPage < totalPages && sliders.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
};
// end fracmento de paginacion
  const [sliderEditado, setSliderEditado] = useState({
    id: null,
    titulo: "",
    descripccion: "",
    foto: "",
  ...
  const getSliders = () => {
    axios
      .get(`${API_URL}/slider`)
      .then((response) => {
        const sortedSliders = response.data.sort();
        setSliders(sortedSliders.reverse());
      const filtered = response.data.filter((slider) => {
        const tituloCompleto = `${slider.titulo} ${slider.descripccion}`;
  ...
  const exportToExcel = (data) => {
    ...
    data.forEach((slider, index) => {
      const row = worksheet.addRow(Object.values(slider));
  ...
  const generarPDF = () => {
    ...
    // Obtener los datos de los sliders en un formato de tabla
  ...
  const editarSlider = (id) => {
    const slider = sliders.find((p) => p.id === id);
    setSliderEditado({
      id: slider.id,
      titulo: slider.titulo,
      descripccion: slider.descripccion,
      foto: slider.foto,
    });
    openModal()
  }

  const crearSlider = async (event) => {
    event.preventDefault();
    if (
      !sliderEditado.titulo.trim() ||
      !sliderEditado.descripccion.trim() 
    ) {
      return;
    }
    ...
    try {
      ...
      // Crear el nuevo slider con la URL de la foto
      const nuevoSlider = {
        titulo: sliderEditado.titulo,
        descripccion: sliderEditado.descripccion,
        foto: fotoUrl,
      };
  
      // Realizar la solicitud POST para crear el slider utilizando axios
      axios
        .post(`${API_URL}/slider`, nuevoSlider)
        .then((response) => {
          setSliderEditado({
            id: null,
            titulo: "",
  ...
  const actualizarSlider = async (event) => {
    event.preventDefault();
  
    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = sliderEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }
  
      // Actualizar los datos del slider, incluyendo la URL de la foto
      const sliderActualizado = {
        id: sliderEditado.id,
        titulo: sliderEditado.titulo,
        descripccion: sliderEditado.descripccion,
        foto: fotoUrl,
      ...
      // Realizar la solicitud PUT para actualizar el slider
      const response = await axios.put(
        `${API_URL}/slider`,
        sliderActualizado
      );
  
      // Actualizar la lista de sliders
      getSliders();
      closeModal()
      setSliderEditado({
        id: null,
  ...
  const eliminarSlider = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);
  
      // Realizar la solicitud DELETE para eliminar el slider
      const response = await axios.delete(`${API_URL}/slider/${id}`);
...
<div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{sliderEditado.id ? 'Editar Slider' : 'Crear Slider'}</h2>
      <form onSubmit={sliderEditado.id ? actualizarSlider : crearSlider}>
        <div className="mt-4">
          <div className="flex">
            <div>
              <label className="block">Titulo</label>
              <input
                value={sliderEditado.titulo}
                onChange={(event) =>
                  setSliderEditado({
                    ...sliderEditado,
            ...
            <div className="ml-1">
              ...
              <input
                value={sliderEditado.descripccion}
                onChange={(event) =>
                  setSliderEditado({
                    ...sliderEditado,
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {sliderEditado.id ? (
                <button onClick={actualizarSlider} >Actualizar</button>
              ) : (
                <button onClick={crearSlider} >Crear </button>
              )}
            </div>
          </div>
        </div>
      ...
      <div class="flex-none w-full max-w-full px-3">
        <div class="flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="dark:text-white font-bold">Lista De Slider</h6>
          </div>
              ...
              <nav class="flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                  <nav className=" flex ">
                    ...
                    <div class="mb-0 font-bold capitalize  m-2 ">
                      <button className="w-10" onClick={() => exportToExcel(sliders)} >
                ...
                <tbody>
                  {currentItems.map((slider) => (
                    <tr key={slider.id}>
                      ...
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{slider.titulo}</p>
                      </td>
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{slider.descripccion}</p>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{slider.created_at}</span>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{slider.updated_at}</span>
                      </td>
                      <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        ...
                        <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                          <button   onClick={() => eliminarSlider(slider.id)} >
                        ...
                        <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                          <button onClick={() => editarSlider(slider.id)} >
