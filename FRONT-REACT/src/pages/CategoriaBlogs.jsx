...
import { saveFile ,  deleteFile} from "../firebase/firebaseCategoriaBlog";
...
import { categoriablog } from "@material-tailwind/react";
const Categoriablogs = () => {
// paginacion
const [currentPage, setCurrentPage] = useState(1);
const [itemsPerPage, setItemsPerPage] = useState(5);
const [searchTerm, setSearchTerm] = useState('');
const [filteredCategoriablogs, setFilteredCategoriablogs] = useState([]);
...
const getCurrentItems = () => {
  ...
  // return Categoriablogs.slice(startIndex, endIndex);
  return filteredCategoriablogs.slice(startIndex, endIndex);
};
...
// end paginacion
  ...
  const totalPages = Math.ceil(categoriablogs.length / itemsPerPage);
 const hasNextPage = () => {
  return currentPage < totalPages && categoriablogs.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
};
// end fracmento de paginacion
  const [categoriablogEditado, setCategoriablogEditado] = useState({
    id: null,
    titulo: "",
    descripccion: "",
    foto: "",
  ...
  const getCategoriablogs = () => {
    axios
      .get(`${API_URL}/categoriablog`)
      .then((response) => {
        const sortedCategoriablogs = response.data.sort();
        setCategoriablogs(sortedCategoriablogs.reverse());
      const filtered = response.data.filter((categoriablog) => {
        const tituloCompleto = `${categoriablog.titulo} ${categoriablog.descripccion}`;
        return tituloCompleto.toLowerCase().includes(searchTerm.toLowerCase());
      });
      
      setFilteredCategoriablogs(filtered);
      })
      .catch((error) => {
        // handle error
        console.log(error);
  ...
  const exportToExcel = (data) => {
    ...
    const headers = ['ID', 'Titulo', 'Descripccion','Foto', 'Creado', 'Actualizado'];
    ...
    data.forEach((categoriablog, index) => {
      const row = worksheet.addRow(Object.values(categoriablog));
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      const filename = 'categoriablogs.xlsx';
  ...
  const generarPDF = () => {
    ...
    // Obtener los datos de los categoriablogs en un formato de tabla
    const data = categoriablogs.map((categoriablog) => [
      categoriablog.id,
      categoriablog.titulo,
      categoriablog.descripccion,
      categoriablog.created_at,
      categoriablog.updated_at,
    ]);
  ...
  const editarCategoriablog = (id) => {
    const categoriablog = categoriablogs.find((p) => p.id === id);
    setCategoriablogEditado({
      id: categoriablog.id,
      titulo: categoriablog.titulo,
      descripccion: categoriablog.descripccion,
      foto: categoriablog.foto,
    });
    openModal()
  }

  const crearCategoriablog = async (event) => {
    event.preventDefault();
    if (
      !categoriablogEditado.titulo.trim() ||
      !categoriablogEditado.descripccion.trim() 
    ) {
      return;
    }
    ...
    try {
      ...
      // Crear el nuevo categoriablog con la URL de la foto
      const nuevoCategoriablog = {
        titulo: categoriablogEditado.titulo,
        descripccion: categoriablogEditado.descripccion,
        foto: fotoUrl,
      };
  
      // Realizar la solicitud POST para crear el categoriablog utilizando axios
      axios
        .post(`${API_URL}/categoriablog`, nuevoCategoriablog)
        .then((response) => {
          setCategoriablogEditado({
            id: null,
            titulo: "",
  ...
  const actualizarCategoriablog = async (event) => {
    event.preventDefault();
  
    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = categoriablogEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }
  
      // Actualizar los datos del categoriablog, incluyendo la URL de la foto
      const categoriablogActualizado = {
        id: categoriablogEditado.id,
        titulo: categoriablogEditado.titulo,
        descripccion: categoriablogEditado.descripccion,
        foto: fotoUrl,
      };
  
      // Realizar la solicitud PUT para actualizar el categoriablog
      const response = await axios.put(
        `${API_URL}/categoriablog`,
        categoriablogActualizado
      );
  
      // Actualizar la lista de categoriablogs
      getCategoriablogs();
      closeModal()
      setCategoriablogEditado({
        id: null,
  ...
  const eliminarCategoriablog = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);
  
      // Realizar la solicitud DELETE para eliminar el categoriablog
      const response = await axios.delete(`${API_URL}/categoriablog/${id}`);
...
<div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{categoriablogEditado.id ? 'Editar Categoriablog' : 'Crear Categoriablog'}</h2>
      <form onSubmit={categoriablogEditado.id ? actualizarCategoriablog : crearCategoriablog}>
        <div className="mt-4">
          <div className="flex">
            <div>
              <label className="block">Titulo</label>
              <input
                value={categoriablogEditado.titulo}
                onChange={(event) =>
                  setCategoriablogEditado({
                    ...categoriablogEditado,
            ...
            <div className="ml-1">
              <label className="block">Descripccion</label>
              <input
                value={categoriablogEditado.descripccion}
                onChange={(event) =>
                  setCategoriablogEditado({
                    ...categoriablogEditado,
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {categoriablogEditado.id ? (
                <button onClick={actualizarCategoriablog} >Actualizar</button>
              ) : (
                <button onClick={crearCategoriablog} >Crear </button>
              )}
            </div>
          </div>
        </div>
      ...
      <div class="flex-none w-full max-w-full px-3">
        <div class=" flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="dark:text-white font-bold">Lista De Categoriablog</h6>
          </div>
          <div class="flex-auto px-0 pt-0 pb-2">
            <div class="p-0 overflow-x-auto ps">
              ...
              <nav class=" flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                  <nav className=" flex ">
                    ...
                    <div class="mb-0 font-bold capitalize  m-2 ">
                      <button className="w-10" onClick={() => exportToExcel(categoriablogs)} >
                    ...
                    <div class="flex items-center md:ml-auto md:pr-4">
                      <div class="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease-soft">
                        <span class="text-sm ease-soft absolute leading-5.6  z-50 -ml-px flex h-full items-center whitespace-nowrap rounded-lg rounded-tr-none rounded-br-none border border-r-0 border-transparent bg-transparent py-2 px-2.5 text-center font-normal text-slate-500 transition-all">
                          <button onClick={getCategoriablogs} >
                            <svg
              ...
              <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                ...
                <tbody>
                  {currentItems.map((categoriablog) => (
                    <tr key={categoriablog.id}>
                      ...
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{categoriablog.titulo}</p>
                      </td>
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{categoriablog.descripccion}</p>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{categoriablog.created_at}</span>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{categoriablog.updated_at}</span>
                      </td>
                      <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        ...
                        <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                          <button   onClick={() => eliminarCategoriablog(categoriablog.id)} >
                        ...
                        <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                          <button onClick={() => editarCategoriablog(categoriablog.id)} >
