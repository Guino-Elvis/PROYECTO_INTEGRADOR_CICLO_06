...
const Alumnos = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const filtered = response.data.filter((alumno) => {
          const nombreCompleto = `${alumno.nombre}  ${alumno.codigo} ${alumno.apellido}  ${alumno.correo}`;
  ...
  const exportToExcel = (data) => {
    ...
    data.forEach((alumno, index) => {
      const row = worksheet.addRow(Object.values(alumno));
  ...
  const generarPDF = () => {
    ...
    // Obtener los datos de los alumnos en un formato de tabla
  ...
  const crearAlumno = async (event) => {
    ...
    if (
      !alumnoEditado.nombre.trim() ||
      !alumnoEditado.apellido.trim() ||
      !alumnoEditado.dni.trim() ||
      !alumnoEditado.codigo.trim() ||
      !alumnoEditado.correo.trim()
    ) {
      return;
    }
    ...
    try {
      ...
      // Crear el nuevo alumno con la URL de la foto
      const nuevoAlumno = {
        nombre: alumnoEditado.nombre,
        apellido: alumnoEditado.apellido,
        codigo: alumnoEditado.codigo,
        dni: alumnoEditado.dni,
        correo: alumnoEditado.correo,
        foto: fotoUrl,
      };

      // Realizar la solicitud POST para crear el alumno utilizando axios
      axios
        .post(`${API_URL}/alumno`, nuevoAlumno,{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const actualizarAlumno = async (event) => {
    ...
    try {
      ...
      let fotoUrl = alumnoEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }

      // Actualizar los datos del alumno, incluyendo la URL de la foto
  ...
  const eliminarAlumno = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);

      // Realizar la solicitud DELETE para eliminar el alumno
      const response = await axios.delete(`${API_URL}/alumno/${id}`,
      {
        headers: {
          Authorization: `Bearer ${token}`,
        },
  ...
  const contenidoModal = (
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{alumnoEditado.id ? 'Editar Alumno' : 'Crear Alumno'}</h2>
      <form onSubmit={alumnoEditado.id ? actualizarAlumno : crearAlumno}>
        <div className="mt-4">
          <div className="flex">
            <div>
      ...
      <div class="flex flex-wrap -mx-3">
        ...
        <div class=" flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
              <h6 class="dark:text-white font-bold">Lista De Alumnos</h6>
            </div>
              ...
              <nav class=" flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                  <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                    <nav className=" flex ">
                      ...
                      <div class="mb-0 font-bold capitalize  m-2 ">
                        <button className="w-10" onClick={() => exportToExcel(alumnos)} >
                        ...
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <div class="flex px-2 py-1">
                            <div>
                              ...
                              />                            </div>
                            <div class="flex flex-col justify-center">
                              <h6 class="mb-0 text-sm leading-normal dark:text-white">{alumno.nombre} {alumno.apellido}</h6>
                              <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{alumno.correo}</p>
                            </div>
                          </div>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{alumno.dni}</p>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{alumno.created_at}</span>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{alumno.updated_at}</span>
                        </td>
