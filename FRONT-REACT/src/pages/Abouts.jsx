...
// import { saveFile, getFile, deleteFile } from "./firebaseUtils";
...
import { about } from "@material-tailwind/react";
const Abouts = () => {
// paginacion
...
const getCurrentItems = () => {
  ...
  // return Abouts.slice(startIndex, endIndex);
  return filteredAbouts.slice(startIndex, endIndex);
};
...
// end paginacion
 ...
 const hasNextPage = () => {
  return currentPage < totalPages && abouts.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
};
// end fracmento de paginacion
  const [aboutEditado, setAboutEditado] = useState({
    id: null,
    nombre: "",
    descripccion: "",
    subdescripccion: "",
  ...
  const getAbouts = () => {
    axios
      .get(`${API_URL}/about`)
      .then((response) => {
        const sortedAbouts = response.data.sort();
        setAbouts(sortedAbouts.reverse());
      const filtered = response.data.filter((about) => {
        const nombreCompleto = `${about.nombre} ${about.descripccion}`;
  ...
  const exportToExcel = (data) => {
    ...
    data.forEach((about, index) => {
      const row = worksheet.addRow(Object.values(about));
  ...
  const generarPDF = () => {
    ...
    // Obtener los datos de los abouts en un formato de tabla
    const data = abouts.map((about) => [
      about.id,
      about.nombre,
      about.descripccion,
      about.subdescripccion,
      about.created_at,
      about.updated_at,
    ]);
  ...
  const editarAbout = (id) => {
    const about = abouts.find((p) => p.id === id);
    setAboutEditado({
      id: about.id,
      nombre: about.nombre,
      descripccion: about.descripccion,
      subdescripccion: about.subdescripccion,
      foto: about.foto,
    });
    openModal()
  }

  const crearAbout = async (event) => {
    event.preventDefault();
    if (
      !aboutEditado.nombre.trim() ||
      !aboutEditado.descripccion.trim() 
    ) {
      return;
    }
    ...
    try {
      ...
      // Crear el nuevo about con la URL de la foto
      const nuevoAbout = {
        nombre: aboutEditado.nombre,
        descripccion: aboutEditado.descripccion,
        subdescripccion: aboutEditado.subdescripccion,
        foto: fotoUrl,
      };
  
      // Realizar la solicitud POST para crear el about utilizando axios
      axios
        .post(`${API_URL}/about`, nuevoAbout)
        .then((response) => {
          setAboutEditado({
            id: null,
            nombre: "",
  ...
  const actualizarAbout = async (event) => {
    event.preventDefault();
  
    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = aboutEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }
  
      // Actualizar los datos del about, incluyendo la URL de la foto
      const aboutActualizado = {
        id: aboutEditado.id,
        nombre: aboutEditado.nombre,
        descripccion: aboutEditado.descripccion,
        subdescripccion: aboutEditado.subdescripccion,
      ...
      // Realizar la solicitud PUT para actualizar el about
      const response = await axios.put(
        `${API_URL}/about`,
        aboutActualizado
      );
  
      // Actualizar la lista de abouts
      getAbouts();
      closeModal()
      setAboutEditado({
        id: null,
  ...
  const eliminarAbout = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);
  
      // Realizar la solicitud DELETE para eliminar el about
...
<div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{aboutEditado.id ? 'Editar About' : 'Crear About'}</h2>
      <form onSubmit={aboutEditado.id ? actualizarAbout : crearAbout}>
        <div className="mt-4">
          <div className="flex">
            <div>
              <label className="block">Titulo</label>
              <input
                value={aboutEditado.nombre}
                onChange={(event) =>
                  setAboutEditado({
                    ...aboutEditado,
            ...
            <div className="ml-1">
              ...
              <input
                value={aboutEditado.descripccion}
                onChange={(event) =>
                  setAboutEditado({
                    ...aboutEditado,
            ...
            <div className="ml-1 pt-2">
              <label className="block">Sub Descripccion</label>
              <input
                value={aboutEditado.subdescripccion}
                onChange={(event) =>
                  setAboutEditado({
                    ...aboutEditado,
                    subdescripccion: event.target.value,
                  })
                }
                type="text"
                placeholder="sub descripccion"
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {aboutEditado.id ? (
                <button onClick={actualizarAbout} >Actualizar</button>
              ) : (
                <button onClick={crearAbout} >Crear </button>
              )}
            </div>
          </div>
        </div>
      ...
      <div class="flex-none w-full max-w-full px-3">
        <div class=" flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="dark:text-white font-bold">Lista De About</h6>
          </div>
              ...
              <nav class=" flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                  <nav className=" flex ">
                    ...
                    <div class="mb-0 font-bold capitalize  m-2 ">
                      <button className="w-10" onClick={() => exportToExcel(abouts)} >
                ...
                <tbody>
                  {currentItems.map((about) => (
                    <tr key={about.id}>
                      ...
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{about.nombre}</p>
                      </td>
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{about.descripccion}</p>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{about.created_at}</span>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{about.updated_at}</span>
                      </td>
                      <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        ...
                        <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                          <button   onClick={() => eliminarAbout(about.id)} >
                        ...
                        <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                          <button onClick={() => editarAbout(about.id)} >
