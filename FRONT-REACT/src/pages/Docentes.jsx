...
import { saveFile, deleteFile } from "../firebase/firebaseDocente";// Importa los métodos de Firebase
import { useState, useEffect } from "react";
import jsPDF from 'jspdf';
import 'jspdf-autotable';
import docentea from '../styles/add.css';
...
const Docentes = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const filtered = response.data.filter((docente) => {
          const nombreCompleto = `${docente.nombre}  ${docente.telefono} ${docente.apellido}  ${docente.correo}`;
  ...
  const exportToExcel = (data) => {
    ...
    const headers = ['ID', 'Código', 'Nombre', 'Apellido', 'DNI', 'Correo', 'Foto', 'Creado', 'Actualizado'];
    ...
    data.forEach((docente, index) => {
      const row = worksheet.addRow(Object.values(docente));
  ...
  const generarPDF = () => {
    ...
    const headers = ['Código', 'Nombre', 'Apellido', 'DNI', 'Correo', 'Creado', 'Actualizado'];

    // Obtener los datos de los docentes en un formato de tabla
  ...
  const crearDocente = async (event) => {
    ...
    if (
      !docenteEditado.nombre.trim() ||
      !docenteEditado.apellido.trim() ||
      !docenteEditado.dni.trim() ||
      !docenteEditado.telefono.trim() ||
      !docenteEditado.correo.trim()
    ) {
      return;
    }
    ...
    try {
      ...
      // Crear el nuevo docente con la URL de la foto
      const nuevoDocente = {
        nombre: docenteEditado.nombre,
        apellido: docenteEditado.apellido,
        telefono: docenteEditado.telefono,
        dni: docenteEditado.dni,
        correo: docenteEditado.correo,
        foto: fotoUrl,
      };

      // Realizar la solicitud POST para crear el docente utilizando axios
      axios
        .post(`${API_URL}/docente`, nuevoDocente)
        .then((response) => {
          setDocenteEditado({
            id: null,
            nombre: "",
  ...
  const actualizarDocente = async (event) => {
    event.preventDefault();

    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = docenteEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }

      // Actualizar los datos del docente, incluyendo la URL de la foto
      const docenteActualizado = {
        id: docenteEditado.id,
        nombre: docenteEditado.nombre,
        apellido: docenteEditado.apellido,
        telefono: docenteEditado.telefono,
        dni: docenteEditado.dni,
      ...
      // Realizar la solicitud PUT para actualizar el docente
  ...
  const eliminarDocente = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);

      // Realizar la solicitud DELETE para eliminar el docente
      const response = await axios.delete(`${API_URL}/docente/${id}`);
  ...
  const contenidoModal = (
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{docenteEditado.id ? 'Editar Docente' : 'Crear Docente'}</h2>
      <form onSubmit={docenteEditado.id ? actualizarDocente : crearDocente}>
        <div className="mt-4">
          <div className="flex">
            <div>
      ...
      <div class="flex flex-wrap -mx-3">
        ...
        <div class=" flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
              <h6 class="dark:text-white font-bold">Lista De Docentes</h6>
            </div>
                ...
                <nav class=" flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                  <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                    <nav className=" flex ">
                      ...
                      <div class="mb-0 font-bold capitalize  m-2 ">
                        <button className="w-10" onClick={() => exportToExcel(docentes)} >
                ...
                <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                  <thead class="align-bottom">
                    <tr>
                      ...
                      <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400   ">Codigo</th>
                      <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400   ">Author</th>
                        ...
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <div class="flex px-2 py-1">
                            <div>
                              ...
                              />                            </div>
                            <div class="flex flex-col justify-center">
                              <h6 class="mb-0 text-sm leading-normal dark:text-white">{docente.nombre} {docente.apellido}</h6>
                              <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{docente.correo}</p>
                            </div>
                          </div>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{docente.dni}</p>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{docente.created_at}</span>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{docente.updated_at}</span>
                        </td>
