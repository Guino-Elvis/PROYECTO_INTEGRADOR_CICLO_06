...
import { saveFile ,  deleteFile} from "../firebase/firebaseCurso";
import SelectDocente from '../components/SelectDocente';
...
// end fracmento de paginacion
  ...
  const getCursos = () => {
    axios
      ...
      .then((response) => {
        ...
        // Obtener los nombres de los docentes para cada curso
        const docentePromises = filtered.map((curso) => {
          return axios.get(`${API_URL}/docente/${curso.docenteId}`);
        });
  
        // Realizar las solicitudes en paralelo
        axios.all(docentePromises)
          .then((responses) => {
            // Mapear los nombres de los docentes a los cursos correspondientes
            const cursosConDocentes = filtered.map((curso, index) => {
              const nombreDocente = responses[index].data.nombre;
              const apellidoDocente = responses[index].data.apellido;
              const correoDocente = responses[index].data.correo;
              const fotoDocente = responses[index].data.foto;
              return { ...curso, nombreDocente,apellidoDocente,correoDocente,fotoDocente };
            });
  
            setFilteredCursos(cursosConDocentes);
          })
          .catch((error) => {
            console.log(error);
          });
  ...
  const exportToExcel = (data) => {
    ...
    const headers = ['ID',  'Costo',  'Nombre', 'Docente Nombre', 'Docente Apellido', 'Docente Correo', 'Foto', 'Creado', 'Actualizado'];
    ...
    data.forEach((curso, index) => {
      const row = worksheet.addRow(Object.values(curso));
  ...
  const generarPDF = () => {
    ...
    const headers = ['ID',  'Costo',  'Nombre', 'Docente Nombre', 'Docente Apellido', 'Docente Correo', 'Creado', 'Actualizado'];
    // Obtener los datos de los cursos en un formato de tabla
  ...
  const crearCurso = async (event) => {
    ...
    if (
      !cursoEditado.nombre.trim() ||
      !cursoEditado.costo.trim() ||
      !cursoEditado.docenteId.trim()
    ...
    try {
      ...
      // Crear el nuevo curso con la URL de la foto
      const nuevoCurso = {
        nombre: cursoEditado.nombre,
        costo: cursoEditado.costo,
        docenteId: cursoEditado.docenteId,
        foto: fotoUrl,
      };
  
      // Realizar la solicitud POST para crear el curso utilizando axios
      axios
        .post(`${API_URL}/curso`, nuevoCurso)
        .then((response) => {
          setCursoEditado({
            id: null,
            nombre: "",
  ...
  const actualizarCurso = async (event) => {
    event.preventDefault();
  
    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = cursoEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }
  
      // Actualizar los datos del curso, incluyendo la URL de la foto
      ...
      // Realizar la solicitud PUT para actualizar el curso
  ...
  const eliminarCurso = async (id, foto) => {
    try {
  ...
  const contenidoModal = (
    ...
    <h2 className="text-2xl font-bold text-center">{cursoEditado.id ? 'Editar Curso' : 'Crear Curso'}</h2>
    <form onSubmit={cursoEditado.id ? actualizarCurso : crearCurso}>
      <div className="mt-4">
        <div className="flex">
          <div>
          ...
          <div className="ml-1">
            ...
            <SelectDocente onChange={(docenteId) =>
                setCursoEditado({
                  ...cursoEditado,
                  docenteId: docenteId,
                })
              }
              selectedDocenteId={cursoEditado.docenteId}
        ...
        <div class="flex-none w-full max-w-full px-3">
          <div class="flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
              <h6 class="dark:text-white font-bold">Lista De Cursos</h6>
            </div>
                ...
                <nav class=" flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                  <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                    <nav className=" flex ">
                      ...
                      <div class="mb-0 font-bold capitalize  m-2 ">
                        <button className="w-10" onClick={() => exportToExcel(cursos)} >
                ...
                <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                  <thead class="align-bottom">
                    <tr>
                      ...
                      <th class="px-6 py-3 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">Foto del Curso</th>
                  ...
                  {currentItems.map((curso)  => (
                      <tr key={curso.id}>
                        ...
                        <td class="p-2 pl-6 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <div>
                              <img
                                src={curso.foto}
                                className="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-20 w-20 rounded-xl"
                                alt="user1"
                              /> </div>
                        </td>
                        <td class="pl-4 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <div class="flex px-2 py-1">
                            <div class="flex flex-col justify-center">
                              <h6 class="mb-0 text-sm leading-normal dark:text-white">{curso.nombre} </h6>
                            </div>
                          </div>
                        </td>
                        ...
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <div class="flex px-2 py-1">
                            <div>
                              <img
                                src={curso.fotoDocente}
                                className="inline-flex items-center justify-center mr-4 text-sm text-white transition-all duration-200 ease-in-out h-9 w-9 rounded-xl"
                                alt="user1"
                              />                            </div>
                            <div class="flex flex-col justify-center">
                              <h6 class="mb-0 text-sm leading-normal dark:text-white">{curso.nombreDocente} ,{curso.apellidoDocente} </h6>
                              <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{curso.correoDocente}</p>
                            </div>
                          </div>
                        </td>
                        <td class="pl-4 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <div class="flex px-2 py-1">
                            <div class="flex flex-col justify-center">
                              <h6 class="mb-0 text-sm leading-normal dark:text-white">S/. {curso.costo} </h6>
                            </div>
                          </div>
                        </td>
                        <td class="pl-12 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{curso.created_at}</span>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{curso.updated_at}</span>
                        </td>
