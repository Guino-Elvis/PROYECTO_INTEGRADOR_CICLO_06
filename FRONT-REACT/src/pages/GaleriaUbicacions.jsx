...
import { saveFile ,  deleteFile} from "../firebase/firebaseGaleria";// Importa los métodos de Firebase
import { useState, useEffect } from "react";
import jsPDF from 'jspdf';
import 'jspdf-autotable';
import alumnoa from '../styles/add.css';
import Modal from "../components/Modal";
import { galeriaUbicacion } from "@material-tailwind/react";
import AppLayout from '../components/admin/AppLayout';
const GaleriaUbicacions = () => {
// paginacion
const [currentPage, setCurrentPage] = useState(1);
const [itemsPerPage, setItemsPerPage] = useState(5);
const [searchTerm, setSearchTerm] = useState('');
const [filteredGaleriaUbicacions, setFilteredGaleriaUbicacions] = useState([]);
...
const getCurrentItems = () => {
  ...
  // return galeriaUbicacions.slice(startIndex, endIndex);
  return filteredGaleriaUbicacions.slice(startIndex, endIndex);
};
...
// end paginacion
  ...
  const [galeriaUbicacions, setGaleriaUbicacions] = useState([]);
  //fracmento de paginacion
  const totalPages = Math.ceil(galeriaUbicacions.length / itemsPerPage);
 const hasNextPage = () => {
  return currentPage < totalPages && galeriaUbicacions.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
};
// end fracmento de paginacion
  const [galeriaUbicacionEditado, setGaleriaUbicacionEditado] = useState({
    id: null,
    titulo: "",
    foto: "",
  });

  const getGaleriaUbicacions = () => {
    axios
      .get(`${API_URL}/galeriaubicacion`)
      .then((response) => {
        const sortedGaleriaUbicacions = response.data.sort();
        setGaleriaUbicacions(sortedGaleriaUbicacions.reverse());
      const filtered = response.data.filter((galeriaUbicacion) => {
        const tituloCompleto = `${galeriaUbicacion.titulo}`;
        return tituloCompleto.toLowerCase().includes(searchTerm.toLowerCase());
      });
      
      setFilteredGaleriaUbicacions(filtered);
      })
      .catch((error) => {
        // handle error
        console.log(error);
  ...
  const exportToExcel = (data) => {
    ...
    const headers = ['ID', 'Titulo','Foto', 'Creado', 'Actualizado'];
    ...
    data.forEach((galeriaUbicacion, index) => {
      const row = worksheet.addRow(Object.values(galeriaUbicacion));
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      const filename = 'galeriaUbicacions.xlsx';
  ...
  const generarPDF = () => {
    ...
    const headers = ['ID', 'Titulo', 'Creado', 'Actualizado'];

    // Obtener los datos de los galeriaUbicacions en un formato de tabla
    const data = galeriaUbicacions.map((galeriaUbicacion) => [
      galeriaUbicacion.id,
      galeriaUbicacion.titulo,
      galeriaUbicacion.created_at,
      galeriaUbicacion.updated_at,
    ]);
    ...
    // Guardar el PDF
    doc.save('galeriaUbicacions.pdf');
  };
  ...
  const editarGaleriaUbicacion = (id) => {
    const galeriaUbicacion = galeriaUbicacions.find((p) => p.id === id);
    setGaleriaUbicacionEditado({
      id: galeriaUbicacion.id,
      titulo: galeriaUbicacion.titulo,
      foto: galeriaUbicacion.foto,
    });
    openModal()
  }

  const crearGaleriaUbicacion = async (event) => {
    event.preventDefault();
    if (
      !galeriaUbicacionEditado.titulo.trim() 
    ...
    try {
      ...
      // Crear el nuevo galeriaUbicacion con la URL de la foto
      const nuevoGaleriaUbicacion = {
        titulo: galeriaUbicacionEditado.titulo,
        foto: fotoUrl,
      };
  
      // Realizar la solicitud POST para crear el galeriaUbicacion utilizando axios
      axios
        .post(`${API_URL}/galeriaubicacion`, nuevoGaleriaUbicacion)
        .then((response) => {
          setGaleriaUbicacionEditado({
            id: null,
            titulo: "",
  ...
  const actualizarGaleriaUbicacion = async (event) => {
    event.preventDefault();
  
    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = galeriaUbicacionEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }
  
      // Actualizar los datos del galeriaUbicacion, incluyendo la URL de la foto
      const galeriaUbicacionActualizado = {
        id: galeriaUbicacionEditado.id,
        titulo: galeriaUbicacionEditado.titulo,

        foto: fotoUrl,
      };
  
      // Realizar la solicitud PUT para actualizar el galeriaUbicacion
      const response = await axios.put(
        `${API_URL}/galeriaubicacion`,
        galeriaUbicacionActualizado
      );
  
      // Actualizar la lista de galeriaUbicacions
      getGaleriaUbicacions();
      closeModal()
      setGaleriaUbicacionEditado({
        id: null,
  ...
  const eliminarGaleriaUbicacion = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);
  
      // Realizar la solicitud DELETE para eliminar el galeriaUbicacion
      const response = await axios.delete(`${API_URL}/galeriaubicacion/${id}`);
...
<div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{galeriaUbicacionEditado.id ? 'Editar GaleriaUbicacion' : 'Crear GaleriaUbicacion'}</h2>
      <form onSubmit={galeriaUbicacionEditado.id ? actualizarGaleriaUbicacion : crearGaleriaUbicacion}>
        <div className="mt-4">
          <div className="flex">
            <div>
              <label className="block">Titulo</label>
              <input
                value={galeriaUbicacionEditado.titulo}
                onChange={(event) =>
                  setGaleriaUbicacionEditado({
                    ...galeriaUbicacionEditado,
          ...
          <div className="mt-4">
            ...
            <div className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600">
                {selectedFile ? (
                  ...
                  <div className="flex items-center mt-2">
                    <img
                      src={galeriaUbicacionEditado.foto}
                      alt="Foto actual"
                      className="w-40 h-40 rounded mr-2"
                    />
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {galeriaUbicacionEditado.id ? (
                <button onClick={actualizarGaleriaUbicacion} >Actualizar</button>
              ) : (
                <button onClick={crearGaleriaUbicacion} >Crear </button>
              )}
            </div>
          </div>
        </div>
      ...
      <div class="flex-none w-full max-w-full px-3">
        <div class="flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="dark:text-white font-bold">Lista De galeriaUbicacion</h6>
          </div>
          <div class="flex-auto px-0 pt-0 pb-2">
            <div class="p-0 overflow-x-auto ps">
              ...
              <nav class="flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                  <nav className=" flex ">
                    ...
                    <div class="mb-0 font-bold capitalize  m-2 ">
                      <button className="w-10" onClick={() => exportToExcel(galeriaUbicacions)} >
                    ...
                    <div class="flex items-center md:ml-auto md:pr-4">
                      <div class="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease-soft">
                        <span class="text-sm ease-soft absolute leading-5.6  z-50 -ml-px flex h-full items-center whitespace-nowrap rounded-lg rounded-tr-none rounded-br-none border border-r-0 border-transparent bg-transparent py-2 px-2.5 text-center font-normal text-slate-500 transition-all">
                          <button onClick={getGaleriaUbicacions} >
                            <svg
              ...
              <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                ...
                <tbody>
                  {currentItems.map((galeriaUbicacion) => (
                    <tr key={galeriaUbicacion.id}>
                      ...
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{galeriaUbicacion.titulo}</p>
                      </td>

                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{galeriaUbicacion.created_at}</span>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{galeriaUbicacion.updated_at}</span>
                      </td>
                      <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        ...
                        <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                          <button   onClick={() => eliminarGaleriaUbicacion(galeriaUbicacion.id)} >
                        ...
                        <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                          <button onClick={() => editarGaleriaUbicacion(galeriaUbicacion.id)} >
...
export default GaleriaUbicacions;
