...
// import { saveFile, deleteFile } from "./firebaseFunctions";
import { saveFile, deleteFile } from "../firebase/firebaseCategoriaBlog";
import SelectCategoriaBlog from "../components/SelectCategoriaBlog";
...
import { blog } from "@material-tailwind/react";
const Blogs = () => {
  // paginacion
  ...
  const hasNextPage = () => {
    return currentPage < totalPages && blogs.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
  };
    ...
    axios
      ...
      .then((response) => {
        ...
        const filtered = response.data.filter((blog) => {
          const tituloCompleto = `${blog.titulo} ${blog.descripccion}`;
          return tituloCompleto.toLowerCase().includes(searchTerm.toLowerCase());
        });

        // Obtener los nombres de las categorias para cada blog
        const categoriaBlogPromises = filtered.map((blog) => {
          return axios.get(`${API_URL}/categoriablog/${blog.categoriaBlogId}`);
        });

        // Realizar las solicitudes en paralelo
        axios.all(categoriaBlogPromises)
          .then((responses) => {
            // Mapear los nombres de las categorias a los blog correspondientes
            const blogsConCategoriaBlogs = filtered.map((blog, index) => {
              const tituloCategoriaBlog = responses[index].data.titulo;
              const descripccionCategoriaBlog = responses[index].data.descripccion;
              return { ...blog, tituloCategoriaBlog, descripccionCategoriaBlog };
            });

            setFilteredBlogs(blogsConCategoriaBlogs);
          })
          .catch((error) => {
            console.log(error);
          });
  ...
  const exportToExcel = (data) => {
    ...
    data.forEach((blog, index) => {
      const row = worksheet.addRow(Object.values(blog));
  ...
  const generarPDF = () => {
    ...
    const headers = ['ID', 'Titulo', 'Descripccion', , 'SubDescripccion', 'Creado', 'Actualizado'];

    // Obtener los datos de los blogs en un formato de tabla
    const data = blogs.map((blog) => [
      blog.id,
      blog.titulo,
      blog.descripccion,
      blog.categoriaBlogId,
      blog.created_at,
      blog.updated_at,
    ]);
  ...
  const editarBlog = (id) => {
    ...
    setBlogEditado({
      id: blog.id,
      titulo: blog.titulo,
      descripccion: blog.descripccion,
      categoriaBlogId: blog.categoriaBlogId,
      foto: blog.foto,
    });
    openModal()
  }

  const crearBlog = async (event) => {
    event.preventDefault();
    if (
      !blogEditado.titulo.trim() ||
      !blogEditado.descripccion.trim()
    ) {
      return;
    }
    ...
    try {
      ...
      // Crear el nuevo blog con la URL de la foto
      const nuevoBlog = {
        titulo: blogEditado.titulo,
        descripccion: blogEditado.descripccion,
        categoriaBlogId: blogEditado.categoriaBlogId,
        foto: fotoUrl,
      };

      // Realizar la solicitud POST para crear el blog utilizando axios
      axios
        .post(`${API_URL}/blog`, nuevoBlog)
        .then((response) => {
          setBlogEditado({
            id: null,
            titulo: "",
  ...
  const actualizarBlog = async (event) => {
    event.preventDefault();

    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = blogEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }

      // Actualizar los datos del blog, incluyendo la URL de la foto
      ...
      // Realizar la solicitud PUT para actualizar el blog
  ...
  const eliminarBlog = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);

      // Realizar la solicitud DELETE para eliminar el blog
  ...
  const contenidoModal = (
    ...
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{blogEditado.id ? 'Editar Blog' : 'Crear Blog'}</h2>
      <form onSubmit={blogEditado.id ? actualizarBlog : crearBlog}>
        <div className="mt-4">
            <div>
              <label className="block">Titulo</label>
              <input
                value={blogEditado.titulo}
                onChange={(event) =>
                  setBlogEditado({
                    ...blogEditado,
            ...
            <div className="ml-1">
              ...
              <input
                value={blogEditado.descripccion}
                onChange={(event) =>
                  setBlogEditado({
                    ...blogEditado,
                ...
                className="w-full h-40 px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600"
              />
            </div>
          <div className="ml-1 pt-2">
            <label className="block">Categoria</label>
            <SelectCategoriaBlog onChange={(categoriaBlogId) =>
              setBlogEditado({
                ...blogEditado,
                categoriaBlogId: categoriaBlogId,
              })
            }
              selectedCategoriaBlogId={blogEditado.categoriaBlogId}
              blogEditado={blogEditado}
              setBlogEditado={setBlogEditado}
            />
          </div>
          <div className="mt-4">
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {blogEditado.id ? (
                <button onClick={actualizarBlog} >Actualizar</button>
              ) : (
                <button onClick={crearBlog} >Crear </button>
              )}
            </div>
          </div>
        </div>
        ...
        <div class="flex-none w-full max-w-full px-3">
          <div class="flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
              <h6 class="dark:text-white font-bold">Lista De Blog</h6>
            </div>
                ...
                <nav class=" flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                  <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                    <nav className=" flex ">
                      ...
                      <div class="mb-0 font-bold capitalize  m-2 ">
                        <button className="w-10" onClick={() => exportToExcel(blogs)} >
                    ...
                    <div class="flex   items-center mt-2 grow sm:mt-0 sm:mr-16 md:mr-0 lg:flex  lg:basis-auto">
                      <div class="flex items-center md:ml-auto md:pr-4">
                        <div class="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease-soft">
                          ...
                          <input
                            className="pl-8  text-sm focus:shadow-soft-primary-outline ease-soft w-1/100 leading-5.6 relative -ml-px block min-w-0 flex-auto rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 pr-3 text-gray-700 transition-all placeholder:text-gray-500 focus:border-fuchsia-300 focus:outline-none focus:transition-shadow"
                            type="text"
                            id="searchInput"
                            value={searchTerm}
                  ...
                  <tbody>
                    {currentItems.map((blog) => (
                      <tr key={blog.id}>
                        ...
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{blog.titulo}</p>
                        </td>
                        <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{blog.tituloCategoriaBlog}</p>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{blog.created_at}</span>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{blog.updated_at}</span>
                        </td>
                        <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          ...
                          <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                            <button onClick={() => eliminarBlog(blog.id)} >
                          ...
                          <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                            <button onClick={() => editarBlog(blog.id)} >
