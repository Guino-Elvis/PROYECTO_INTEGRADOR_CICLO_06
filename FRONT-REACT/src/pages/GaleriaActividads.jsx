...
import { galeriaActividad } from "@material-tailwind/react";
import AppLayout from '../components/admin/AppLayout';
const GaleriaActividads = () => {
// paginacion
const [currentPage, setCurrentPage] = useState(1);
const [itemsPerPage, setItemsPerPage] = useState(5);
const [searchTerm, setSearchTerm] = useState('');
const [filteredGaleriaActividads, setFilteredGaleriaActividads] = useState([]);
...
const getCurrentItems = () => {
  ...
  // return galeriaActividads.slice(startIndex, endIndex);
  return filteredGaleriaActividads.slice(startIndex, endIndex);
};
...
// end paginacion
  ...
  const [galeriaActividads, setGaleriaActividads] = useState([]);
  //fracmento de paginacion
  const totalPages = Math.ceil(galeriaActividads.length / itemsPerPage);
 const hasNextPage = () => {
  return currentPage < totalPages && galeriaActividads.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
};
// end fracmento de paginacion
  const [galeriaActividadEditado, setGaleriaActividadEditado] = useState({
    id: null,
    titulo: "",
    foto: "",
  });

  const getGaleriaActividads = () => {
    axios
      .get(`${API_URL}/galeriaactividad`)
      .then((response) => {
        const sortedGaleriaActividads = response.data.sort();
        setGaleriaActividads(sortedGaleriaActividads.reverse());
      const filtered = response.data.filter((galeriaActividad) => {
        const tituloCompleto = `${galeriaActividad.titulo}`;
        return tituloCompleto.toLowerCase().includes(searchTerm.toLowerCase());
      });
      
      setFilteredGaleriaActividads(filtered);
      })
      .catch((error) => {
        // handle error
        console.log(error);
  ...
  const exportToExcel = (data) => {
    ...
    data.forEach((galeriaActividad, index) => {
      const row = worksheet.addRow(Object.values(galeriaActividad));
    ...
    workbook.xlsx.writeBuffer().then((buffer) => {
      ...
      const filename = 'galeriaActividads.xlsx';
  ...
  const generarPDF = () => {
    ...
    // Obtener los datos de los galeriaActividads en un formato de tabla
    const data = galeriaActividads.map((galeriaActividad) => [
      galeriaActividad.id,
      galeriaActividad.titulo,
      galeriaActividad.created_at,
      galeriaActividad.updated_at,
    ]);
    ...
    // Guardar el PDF
    doc.save('galeriaActividads.pdf');
  };
  ...
  const editarGaleriaActividad = (id) => {
    const galeriaActividad = galeriaActividads.find((p) => p.id === id);
    setGaleriaActividadEditado({
      id: galeriaActividad.id,
      titulo: galeriaActividad.titulo,
      foto: galeriaActividad.foto,
    });
    openModal()
  }

  const crearGaleriaActividad = async (event) => {
    event.preventDefault();
    if (
      !galeriaActividadEditado.titulo.trim() 
    ...
    try {
      ...
      // Crear el nuevo galeriaActividad con la URL de la foto
      const nuevoGaleriaActividad = {
        titulo: galeriaActividadEditado.titulo,
        foto: fotoUrl,
      };
  
      // Realizar la solicitud POST para crear el galeriaActividad utilizando axios
      axios
        .post(`${API_URL}/galeriaactividad`, nuevoGaleriaActividad)
        .then((response) => {
          setGaleriaActividadEditado({
            id: null,
            titulo: "",
  ...
  const actualizarGaleriaActividad = async (event) => {
    event.preventDefault();
  
    try {
      // Subir la foto a Firebase Storage si se seleccionó una nueva imagen
      let fotoUrl = galeriaActividadEditado.foto;
      if (selectedFile) {
        fotoUrl = await saveFile(selectedFile);
      }
  
      // Actualizar los datos del galeriaActividad, incluyendo la URL de la foto
      const galeriaActividadActualizado = {
        id: galeriaActividadEditado.id,
        titulo: galeriaActividadEditado.titulo,

        foto: fotoUrl,
      };
  
      // Realizar la solicitud PUT para actualizar el galeriaActividad
      const response = await axios.put(
        `${API_URL}/galeriaactividad`,
        galeriaActividadActualizado
      );
  
      // Actualizar la lista de galeriaActividads
      getGaleriaActividads();
      closeModal()
      setGaleriaActividadEditado({
        id: null,
  ...
  const eliminarGaleriaActividad = async (id, foto) => {
    try {
      // Eliminar la foto de Firebase Storage
      await deleteFile(`images/${foto}`);
  
      // Realizar la solicitud DELETE para eliminar el galeriaActividad
      const response = await axios.delete(`${API_URL}/galeriaactividad/${id}`);
...
<div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{galeriaActividadEditado.id ? 'Editar GaleriaActividad' : 'Crear GaleriaActividad'}</h2>
      <form onSubmit={galeriaActividadEditado.id ? actualizarGaleriaActividad : crearGaleriaActividad}>
        <div className="mt-4">
          <div className="flex">
            <div>
              <label className="block">Titulo</label>
              <input
                value={galeriaActividadEditado.titulo}
                onChange={(event) =>
                  setGaleriaActividadEditado({
                    ...galeriaActividadEditado,
          ...
          <div className="mt-4">
            ...
            <div className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600">
                {selectedFile ? (
                  ...
                  <div className="flex items-center mt-2">
                    <img
                      src={galeriaActividadEditado.foto}
                      alt="Foto actual"
                      className="w-40 h-40 rounded mr-2"
                    />
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {galeriaActividadEditado.id ? (
                <button onClick={actualizarGaleriaActividad} >Actualizar</button>
              ) : (
                <button onClick={crearGaleriaActividad} >Crear </button>
              )}
            </div>
          </div>
        </div>
      ...
      <div class="flex-none w-full max-w-full px-3">
        <div class="flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
          <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
            <h6 class="dark:text-white font-bold">Lista De galeriaActividad</h6>
          </div>
          <div class="flex-auto px-0 pt-0 pb-2">
            <div class="p-0 overflow-x-auto ps">
              ...
              <nav class="flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                  <nav className=" flex ">
                    ...
                    <div class="mb-0 font-bold capitalize  m-2 ">
                      <button className="w-10" onClick={() => exportToExcel(galeriaActividads)} >
                    ...
                    <div class="flex items-center md:ml-auto md:pr-4">
                      <div class="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease-soft">
                        <span class="text-sm ease-soft absolute leading-5.6  z-50 -ml-px flex h-full items-center whitespace-nowrap rounded-lg rounded-tr-none rounded-br-none border border-r-0 border-transparent bg-transparent py-2 px-2.5 text-center font-normal text-slate-500 transition-all">
                          <button onClick={getGaleriaActividads} >
                            <svg
              ...
              <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                ...
                <tbody>
                  {currentItems.map((galeriaActividad) => (
                    <tr key={galeriaActividad.id}>
                      ...
                      <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{galeriaActividad.titulo}</p>
                      </td>

                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{galeriaActividad.created_at}</span>
                      </td>
                      <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{galeriaActividad.updated_at}</span>
                      </td>
                      <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                        ...
                        <div className=" m-1 btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25" href="javascript:;">
                          <button   onClick={() => eliminarGaleriaActividad(galeriaActividad.id)} >
                        ...
                        <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                          <button onClick={() => editarGaleriaActividad(galeriaActividad.id)} >
...
export default GaleriaActividads;
