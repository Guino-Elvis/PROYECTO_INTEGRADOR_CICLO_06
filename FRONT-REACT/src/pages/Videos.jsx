...
const Videos = () => {
  ...
  const hasNextPage = () => {
    return currentPage < totalPages && videos.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
  };
  ...
  //mostrar videe de you tube en el formulario 
  const handleVideoLinkChange = (event) => {
    const videolink = event.target.value;
    setVideoEditado({ ...videoEditado, videolink });
  };
  ...
  //end mostrar videe de you tube en el formulario 
  const getVideos = () => {
    axios
      .get(`${API_URL}/video`)
      .then((response) => {
        ...
        const filtered = response.data.filter((video) => {
          const tituloCompleto = `${video.titulo} ${video.videolink}`;
  ...
  const exportToExcel = (data) => {
    ...
    const headers = ['ID', 'Titulo', 'videolink', 'Creado', 'Actualizado'];
    ...
    data.forEach((video, index) => {
      const row = worksheet.addRow(Object.values(video));
  ...
  const generarPDF = () => {
    ...
    // Obtener los datos de los videos en un formato de tabla
  ...
  const editarVideo = (id) => {
    ...
    setVideoEditado({
      ...
      estudiante: video.estudiante,
      seccion: video.seccion,
      descripccion:video.descripccion,
  ...
  const crearVideo = async (event) => {
    ...
    if (
      !videoEditado.titulo.trim() ||
      !videoEditado.estudiante.trim() ||
      !videoEditado.seccion.trim() ||
      !videoEditado.descripccion.trim() ||
      !videoEditado.videolink.trim()
    ) {
      return;
    }
    ...
    try {
      ...
      const nuevoVideo = {
        ...
        videolink: videoEditado.videolink,
        estudiante: videoEditado.estudiante,
        seccion: videoEditado.seccion,
        descripccion:videoEditado.descripccion,
  

      };

      // Realizar la solicitud POST para crear el video utilizando axios
      axios
        .post(`${API_URL}/video`, nuevoVideo)
        .then((response) => {
          setVideoEditado({
            id: null,
            titulo: "",
  ...
  const eliminarVideo = async (id) => {
    try {
      ...
      // Realizar la solicitud DELETE para eliminar el video
      const response = await axios.delete(`${API_URL}/video/${id}`);
  ...
  const contenidoModal = (
    ...
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{videoEditado.id ? 'Editar Video' : 'Crear Video'}</h2>
      <form onSubmit={videoEditado.id ? actualizarVideo : crearVideo}>
        <div className="mt-4">
        <div className="flex">
            <div>
            ...
            <div className="ml-2">
              <label className="block">Estudiante</label>
              <input
                value={videoEditado.estudiante}
                onChange={(event) =>
                  setVideoEditado({
                    ...videoEditado,
                    estudiante: event.target.value,
                  })
                }
                type="text"
          ...
          <div className="flex">
            <div>
              <label className="block">seccion</label>
              <input
                value={videoEditado.seccion}
                onChange={(event) =>
                  setVideoEditado({
                    ...videoEditado,
            ...
            <div className="ml-2">
              ...
              <input
                value={videoEditado.descripccion}
                onChange={(event) =>
                  setVideoEditado({
                    ...videoEditado,
          ...
          <div className="ml-1">
            <label className="block">videolink</label>
            <input
              value={videoEditado.videolink}
              onChange={handleVideoLinkChange}
              type="text"
              placeholder="videolink"
              className="w-full px-4 py-2 mt-2 border rounded-md focus:outline-none focus:ring-1 focus:ring-blue-600"
            />
            {videoEditado.videolink && (
              <div  className="flex border-spacing-2 p-2 bg-black rounded-xl flex-shrink-0 flex-wrap items-center justify-center rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
                <YouTube videoId={getYouTubeVideoId(videoEditado.videolink)} opts={opts} />
              </div>
            )}
          </div>
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {videoEditado.id ? (
                <button onClick={actualizarVideo} >Actualizar</button>
              ) : (
          ...
          <div class="flex-none w-full max-w-full px-3">
            <div class="flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
              <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
                <h6 class="dark:text-white font-bold">Lista De video</h6>
              </div>
                  ...
                  <nav class="flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                    <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                      <nav className=" flex ">
                        ...
                        <div class="mb-0 font-bold capitalize  m-2 ">
                          <button className="w-10" onClick={() => exportToExcel(videos)} >
                  ...
                  <table class="items-center w-full mb-0 align-top border-collapse dark:border-white/40 text-slate-500">
                    <thead class="align-bottom">
                      <tr>
                        ...
                        <th class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-collapse shadow-none dark:border-white/40 dark:text-white text-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400  ">videolink</th>
                    ...
                    <tbody>
                      {currentItems.map((video) => (
                        <tr key={video.id}>
                          ...
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{video.titulo}</p>
                          </td>
                          <td class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{video.videolink}</p>
                          </td>
                          <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{video.created_at}</span>
                          </td>
                          <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{video.updated_at}</span>
                          </td>
                          <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                            ...
                            <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                              <button onClick={() => editarVideo(video.id)} >
