...
const Horarios = () => {
  ...
  const [filteredhHorarios, setFilteredHorarios] = useState([]);

  const getCurrentItems = () => {
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    // return Horarios.slice(startIndex, endIndex);
    return filteredhHorarios.slice(startIndex, endIndex);
  };
  // end paginacion
  ...
  const hasNextPage = () => {
    return currentPage < totalPages && horarios.slice(currentPage * itemsPerPage, (currentPage + 1) * itemsPerPage).length > 0;
  };
  ...
  const getHorarios = () => {
    axios
      ...
      .then((response) => {
        ...
        setHorarios(sortedHorarios.reverse());
        // Filtrar horarios por nombre
        const filtered = response.data.filter((horario) => {
          const nombreCompleto = `${horario.nombre} ${horario.hora}`;
  ...
  const exportToExcel = (data) => {
    ...
    const headers = ['ID', 'Nombre', 'Hora', 'Creado', 'Actualizado'];
    ...
    data.forEach((horario, index) => {
      const row = worksheet.addRow(Object.values(horario));
  ...
  const generarPDF = () => {
    ...
    const headers = ['ID', 'Nombre', 'Hrora', 'Creado', 'Actualizado'];
  ...
  const crearHorario = (event) => {
    ...
    if (!horarioEditado.nombre.trim() || !horarioEditado.hora.trim()) {
  ...
  const contenidoModal = (
    <div className="px-6 py-6 mx-2 mt-2 mb-2 text-left bg-white   shadow-slate-400 shadow-md">
      ...
      <h2 className="text-2xl font-bold text-center">{horarioEditado.id ? 'Editar Horario' : 'Crear Horario'}</h2>
      <form onSubmit={horarioEditado.id ? actualizarHorario : crearHorario}>
        <div className="mt-4">
          <div className="flex">
            <div>
          ...
          <div className="flex  flex-shrink-0 flex-wrap items-center justify-end rounded-b-md border-t-2 border-neutral-100 border-opacity-100 p-4 dark:border-opacity-50">
            ...
            >
              {horarioEditado.id ? (
                ...
                <button onClick={crearHorario} >Crear </button>
              )}
            </div>
          </div>
        </div>
        ...
        <div class="flex-none w-full max-w-full px-3">
          <div class=" flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
            <div class="p-6 pb-0 mb-0 border-b-0 border-b-solid rounded-t-2xl border-b-transparent">
              <h6 class="dark:text-white font-bold">Lista De Horarios</h6>
            </div>
                ...
                <nav class=" flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start" >
                  <div class="flex  items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit ">
                    <nav className=" flex ">
                      ...
                      <div class="mb-0 font-bold capitalize  m-2 ">
                        <button className="w-10" onClick={() => exportToExcel(horarios)} >
                    ...
                    <div class="flex   items-center mt-2 grow sm:mt-0 sm:mr-16 md:mr-0 lg:flex  lg:basis-auto">
                      <div class="flex items-center md:ml-auto md:pr-4">
                        <div class="relative flex flex-wrap items-stretch w-full transition-all rounded-lg ease-soft">
                          <span class="text-sm ease-soft absolute leading-5  z-50 -ml-px flex h-full items-center whitespace-nowrap rounded-lg rounded-tr-none rounded-br-none border border-r-0 border-transparent bg-transparent py-2 px-2.5 text-center font-normal text-slate-500 transition-all">
                            <button onClick={getHorarios} >
                              <svg
                  ...
                  <tbody>
                    {currentItems.map((horario) => (
                      <tr key={horario.id}>
                        ...
                        <td class="pl-6 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{horario.nombre}</p>
                        </td>
                        <td class="pl-6 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <p class="mb-0 text-xs leading-tight dark:text-white dark:opacity-80 text-slate-400">{horario.hora}</p>
                        </td>
                        <td class="pl-12 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{horario.created_at}</span>
                        </td>
                        <td class="p-2 text-center align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          <span class="text-xs font-semibold leading-tight dark:text-white dark:opacity-80 text-slate-400">{horario.updated_at}</span>
                        </td>
                        <td class=" sticky right-0 p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                          ...
                          <a class="btns inline-block px-4 py-3 mb-0 font-bold text-center uppercase align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 " href="javascript:;">
                            <button onClick={() => editarHorario(horario.id)}>
