...
import 'package:flutter_application_1/src/pages/cruds/libro/Crear_libro.dart';
import 'package:flutter_application_1/src/pages/cruds/libro/Detalle_libro.dart';
...
class LibroList extends StatefulWidget {
  ...
  const LibroList({Key? key}) : super(key: key);
  
  @override
  State<LibroList> createState() => _LibroListtState();
}

class _LibroListtState extends State<LibroList> {
 ...
 Future<List<dynamic>> getData() async {
  ...
  Future<void> _navigateCrearLibro(BuildContext context) async {
    final result = await Navigator.push(context, MaterialPageRoute(builder: (context) => CrearLibro()));
    if (result != null) {
      setState(() {});
    }
  }
                        ...
                        actions: <Widget>[
                          ...
                          TextButton(
                            ...
                            onPressed: () async {
                              Navigator.of(context).pop(); // Cierra el modal
                              try {
                                await libroController.exportDataToExcel();
                        ...
                        actions: <Widget>[
                          ...
                          TextButton(
                            ...
                            onPressed: () async {
                              Navigator.of(context).pop(); // Cierra el modal
                              try {
                                await libroController.exportDataToPDF();
...
class ItemList extends StatelessWidget {
  ...
  Widget build(BuildContext context) => ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final autor = truncateString(item['autor']?.toString() ?? 'autor no especificado', 15);
          final isbn = truncateString(item['isbn']?.toString() ?? 'isbn no especificado', 15);
          final descripcion = truncateString(item['descripcion']?.toString() ?? 'descripcion no especificado', 15);
          final disponibilidad = truncateString(item['disponibilidad']?.toString() ?? 'disponibilidad no especificado', 15);
          final formato = truncateString(item['formato']?.toString() ?? 'formato no especificado', 15);
          final estado = truncateString(item['descripcion']?.toString() ?? 'titulo no especificado', 13);
                                      ...
                                      trailing: IconButton(
                                        ...
                                        onPressed: () {
                                          Navigator.of(context).push(
                                            MaterialPageRoute(
                                              builder: (BuildContext context) => DetalleLibro(list: list, index: index),
                                            ),
                                          );
                                        },
                                      ),
                                                  ...
                                                  actions: [
                                                    ...
                                                    TextButton(
                                                      onPressed: () {
                                                        ...
                                                        libroController.removerLibro(id, fotoURL).then((response) async {
                                                          if (response.statusCode == 200) {
                                                            ScaffoldMessenger.of(scaffoldContext).showSnackBar(SnackBar(content: Text("Item eliminado con éxito.")));
                                                            // actualizarVista([]);
                                                              Navigator.of(scaffoldContext).pushNamed('/libro_crud');  // Navega a la otra ruta
